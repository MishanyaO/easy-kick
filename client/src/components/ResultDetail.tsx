...
function nearest(elapsed: number[], target: number): number {
  ...
  for (let i = 1; i < elapsed.length; i++) {
    if (Math.abs(elapsed[i] - target) < Math.abs(elapsed[best] - target))
      best = i
  }
  return best
}
...
function Tile({
  ...
  return (
    <div className="flex min-w-0 flex-col rounded-sm bg-[var(--bg-surface)] px-2.5 py-2">
      <div className="text-label font-bold tracking-[0.18em] text-[var(--text-muted)]">
        {label}
      </div>
...
function WindowChart({
  ...
  const d = visible
    .map(
      (v, j) =>
        `${j === 0 ? "M" : "L"}${x(lo + j).toFixed(1)},${y(v).toFixed(1)}`,
    )
    .join(" ")
    ...
    <div>
      ...
      >
        <rect
          x={x(startIdx)}
          y={0}
          width={Math.max(1, x(closeIdx) - x(startIdx))}
          height={CH}
          fill={tint}
          opacity={0.13}
        />
      ...
      <div
        className="mt-1 flex items-baseline gap-1.5 text-label"
...
function Votes({
  ...
  return (
    <div className="space-y-1">
      {keys.map((k) => {
        ...
        return (
          <div key={k} className="flex items-center gap-2">
            <span
              className="w-20 shrink-0 truncate text-label"
              style={{
      ...
      <p
        className="tnum text-label text-[var(--text-muted)]"
...
/**
 * The other half of the window: the picture that was running through it.
 *
 * Everything else in this panel is read off chat, because chat is all the policy gets — and
 * the screen is usually the actual cause of whatever chat did. Showing the frame next to the
 * transcript is what lets a streamer see the two together.
 *
 * The frame is the same asset the Stream Preview panel shows while live, deliberately: the
 * two surfaces are looking at one stream and must not disagree about what was on it.
 *
 * What this is NOT is a per-window capture. There is one stream frame for the whole session,
 * so it is honest for a demo — the stream really was showing this the entire time — and it
 * would be a lie the moment there is a real video source, where each window needs its own
 * grab. The metadata below stays empty for the same reason: the frame is a backdrop, the
 * scene and the last event are still not captured, and filling them in from the current
 * moment would put a measurement-shaped number on something nobody measured.
 */
function StreamContext() {
  return (
    <div
      className="flex h-full min-h-[160px] flex-col rounded-md border border-dashed border-[var(--border)] p-2.5 text-[var(--text-muted)]"
      title="Gambit reads the room, not the screen. Carrying real per-window stream context is next — a quiet minute on a loading screen is not the same lull as a quiet minute mid-fight."
    >
      {/* The frame takes the slack, so the tile grows next to a long transcript without the
          rows below drifting apart from each other. */}
      <div className="relative min-h-[96px] flex-1 overflow-hidden rounded-sm bg-black">
        <img
          src="/live-stream-hidden.png"
          alt="The stream frame while this window ran"
...
export default function ResultDetail({
  ...
  const at =
    h.elapsed[r.tick] != null
      ? clock(Math.max(0, h.elapsed[r.tick] - WINDOW_S))
      : null
  ...
  return (
    <div className="border-t border-[var(--border)] bg-[var(--bg-elevated)] px-3 py-2.5">
      ...
      <div className="mt-2 flex items-baseline gap-2 text-label text-[var(--text-muted)]">
        {caveat ? (
          <span className="min-w-0 truncate text-[var(--warn)]" title={caveat}>
            {caveat}
          </span>
        ) : (
          <span
          ...
          >
            measured against {r.controls} quiet{" "}
            {r.controls === 1 ? "window" : "windows"} like this one
          </span>
        )}
        {why && (
        ...
        >
          <ChevronRight
            size={10}
            style={{
              transform: nerd ? "rotate(90deg)" : undefined,
          ...
          {(
            [
              [
                ...
                "against comparable quiet windows",
              ],
              [
                "naive lift",
                points(r.lift_naive),
                "before/after only, biased by drift",
              ],
              [
                "reward",
                r.reward.toFixed(3),
                "the [0,1] squash that updates the posterior",
              ],
              [
                "controls",
                String(r.controls),
                "clean same-state quiet windows averaged",
              ],
              [
                "propensity",
                r.action ? r.action.propensity.toFixed(2) : "—",
              ...
              [
                "standing",
                why?.text ?? "—",
                "where this tactic sits in this state now",
              ],
            ] as const
          ).map(([k, v, hint]) => (
