...
indirect, via the streamer acting on it. Zero API risk, cannot fail on stage, and never
competes for a posterior — it's bandit-adjacent, not bandit-selected. `chat_digest` is the
implemented instance: a card surfacing a question several people are asking that's easy to
miss in a fast-moving chat. Rule-triggered each tick (subject to the same cooldown/hourly cap
as everything else so it can't collide with a fired arm's window), never posted to chat,
never scored.
...
| `quiz` | a quick trivia question, tallies answers over 60s the same way `chat_poll` does | one keystroke + reading | semi-engaged |
| `nothing` | — | — | — |

`chat_poll` and `quiz` are a real poll without a poll API: the bot asks, chat replies, the bot
counts. Real votes, real reaction, fully measurable.

The demo-critical arms are `nothing`, `emote_rally` and `chat_poll`. `question_relay` and
`shoutout` were cut entirely rather than held in reserve — `question_relay`'s detection logic
lives on, generalized, as `chat_digest`'s Tier 0 eligibility check.

**Arms must be behaviourally distinct or the bandit cannot separate them in ~40 decisions.**
The axis we span is *cost of participation*, because that decides who converts. Arms that all
cost chat the same effort would give us a demo where nothing differentiates.
...
**Cut order if behind:** `quiz`, then native `prediction`. Two active arms plus `nothing` tell
the whole story.
...
- **Eligibility is resolved before selection.** A `chat_digest` without a buried question to
  surface, and any capped or disabled arm, are absent from the choice set and propensity.
...
| Arm | Default | Why |
|---|---|---|
| `emote_rally` | `auto`, after setup consent | Spends nothing but a chat line |
| `chat_poll`, `quiz` | `ask` → `auto` after N approvals | Occupies chat's attention |
| `prediction` | `ask`, always | Stakes *viewers'* Channel Points. Not ours to spend |
| `chat_digest` | `auto` | Never posts to chat, so there's nothing for the streamer to approve |
...
`chat_digest`, a rule rather than something to learn. Recent transcript → content generation
...
`controller.result`, `controller.bandit`, `controller.context`, `controller.digest` — whose
`payload` is already the frontend shape. `EventEnvelope.type` is a plain `str`, so these coexist
with `EventType` without touching `models.py`. `read.py::_chat_sse` grows a dispatch: chat events
keep the existing path, `controller.*` events serialise `event.payload` directly, everything
else is still dropped.

```ts
// client/src/types.ts
export type Arm = 'nothing' | 'emote_rally' | 'chat_poll' | 'quiz'
                | 'chat_digest' | 'prediction';
...
**Cut lines, in order.** 1. Audio context provider. 2. Native prediction. 3. `quiz`.
4. LLM copy → templates. 5. Estimator scatterplot. Keep one live Kick action, the three-arm
loop and the streamer controls.
